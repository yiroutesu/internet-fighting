...
namespace GamesMing
{
    [CustomEditor(typeof(TestSOD))]
    public class EditorTestSOD : Editor
    {
        private TestSOD tar;
        ...
        private float starty = 110;// y坐标起始量, 手动设置
        ...
        {
            tar = target as TestSOD;
        }

        public override void OnInspectorGUI()
        {
            base.OnInspectorGUI();
            EditorGUILayout.Space(250);// 腾出空白空间用来画曲线
            //return;
            ...
            SOD.SetInitState(0);
            SOD.SetConstants(tar.f, tar.z, tar.r);
            float time = 0;
            while (time <= maxTime)
            {
                time += delTime;
