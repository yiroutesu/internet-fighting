...
namespace GamesMing
{
    ...
    完成腿部速度与身体旋转速度匹配
   */
    public class SpiderIkController_V_2 : MonoBehaviour
    {
        #region 用于观察内部数值的字段
        ...
        public float LiftHeight = 0.2f;// 抬起高度
        public float LegSpeedFactor = 3;// 腿速度因子
        public float LegMinSpeed = 4;// 腿部最小速度
        ...
        private void CheckStep(SpiderLeg leg, Vector3 delmove)
        {
            ...
            if (dis > ConstraintRadiusMin)// 最小约束，可以尝试移动
            {
                if (dis > ConstraintRadiusMax)// 最大约束，直接移动
                    ...
                    return;
                ATU:;//  重新计算落脚点, 并开始抬脚
                Vector3 dir = delmove.magnitude >= 0.005f ? delmove.normalized : Vector3.zero;// 根据身体当前移动方向判断，补正落脚点方向
                float correction = GetCorrection();// 脚步补正距离（如人向前走，落脚点会在身体中线往前一点距离）
                Ray ray = new Ray(center + dir * correction + body.up, -body.up);
                //Debug.DrawLine(center + dir * correction + body.up, center + dir * correction - body.up, Color.red, 0.5f);// 落脚点射线可视化
                if (Physics.Raycast(ray, out RaycastHit info, 5, CollsionLayer))
                {
        ...
        { // 根据身体的移动速度，判断是否进行落脚点补正，并对最大补正距离限制
            float result = 0;
            float temp = body_SOD_m.YD.magnitude;
            result = Mathf.Min(temp > 0.2f ? ConstraintRadiusMin + temp * 0.1f : 0, ConstraintRadiusMax);
        ...
        {
            float angleDifference = Mathf.Abs(body.localEulerAngles.y - lastYRotate);// 角度变化
            lastYRotate = body.localEulerAngles.y;
            float radius = new Vector2(legSpacing.x, legSpacing.z).magnitude;// 半径
            float speedc = Mathf.PI * radius * angleDifference / 180f / Time.deltaTime;
            LegSpeed = Mathf.Max(LegSpeed + speedc * deltime * 5 * LegSpeedFactor, LegMinSpeed);
        }
