...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    public float maxPush = 0.5f;   // 0.5 m 足够
    ...
    Vector3[] MatchToSurfaceFromAbove(Vector3 point, float halfRange, Vector3 up, LayerMask layer)
    {
        Vector3[] res = new Vector3[2];
        Ray ray = new Ray(point + halfRange * up / 2f, -up);
        if (Physics.SphereCast(ray, sphereCastRadius, out RaycastHit hit,
                               2f * halfRange, layer, QueryTriggerInteraction.Ignore))
        {
            res[0] = hit.point;
            res[1] = hit.normal;
        }
    ...
    void FixedUpdate()
    {
        ...
        if (indexToMove != -1 && !legMoving[0])
        {
            ...
            Vector3 targetPoint = desiredPositions[indexToMove]
                      ...
                      + velocity.normalized * push;   // 方向保留，距离受限
            Vector3[] positionAndNormalFwd = MatchToSurfaceFromAbove(targetPoint, raycastRange,
                                               (transform.parent.up - velocity * 100).normalized,
                                               groundLayer);
            Vector3[] positionAndNormalBwd = MatchToSurfaceFromAbove(targetPoint + velocity * velocityMultiplier, raycastRange * (1f + velocity.magnitude), (transform.parent.up + velocity * 75).normalized, groundLayer);

            legMoving[0] = true;

            if (positionAndNormalFwd[1] == Vector3.zero)
            {
                targetPoint.y = transform.position.y - raycastRange; 
